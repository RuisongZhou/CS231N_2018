...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        #######################################################################
        sample_mean = np.mean(x,axis = 0)  ##均值
        sample_var = np.sum((x-sample_mean)**2,axis=0) /N   ##标准差
        X_nor = (x-sample_mean)/np.sqrt(sample_var+eps)   ##中心化，归一化
        
        out = gamma*X_nor + beta
        cache = (x, eps, gamma, beta, X_nor, sample_mean, sample_var)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for ni in range(x.shape[0]):  # number of samples
        for fi in range(w.shape[0]): # number of filters
            for xi in range(H_): # size of a simple after padding
                for yi in range(W_):
...
def conv_backward_naive(dout, cache):
    ...
    Returns a tuple of:
    - dx: Gradient with respect to x
    - dw: Gradient with respe.ct to w
